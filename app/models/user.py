...
class User(DBItem, UserMixin):
    ...
    first_name = db.Column(db.String(MAX_FIRST_NAME_LEN),
                           nullable=False)
    last_name = db.Column(db.String(MAX_LAST_NAME_LEN),
                          nullable=False)
    email = db.Column(db.String(MAX_EMAIL_LEN), index=True,
    ...
    def has_moderator_rights(self) -> bool:
        """Checks if user has moderator rights"""
        return bool(self.role <= UserRole.MODERATOR)
...
class Invitation(DBItem):
    ...
    name = db.Column(db.String(MAX_FIRST_NAME_LEN +
                               MAX_LAST_NAME_LEN+1),
                     nullable=False)
